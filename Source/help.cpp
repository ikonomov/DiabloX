...
const char *const ShrineListText[] = {
	N_("$Shrines:"),
	...
	N_("Enchanted: -1 slvl to one known spell, +1 to all others"),
	N_("Fascinating: lose 2% of base magic, +2 slvl to Firebolt"),
	N_("Glimmering: identifies all items"),
	N_("Gloomy: +2 armor class to all armor, -1 max damage to all weapons"),
	...
	N_("Ornate: lose 2% of base magic, +2 slvl to Holy Bolt"),
	N_("Quiet: +1 vitality"),
	N_("Religious: restores durability of all items"),
	N_("Sacred: lose 2% of base magic, +2 slvl to Charged Bolt"),
...
void InitHelp()
{
	...
	HelpFlag = false;
	if (MyPlayer->pDiabloKillLevel == 0) {
		...
		for (const auto *text : ShrineListText) {
			const std::string paragraph = WordWrapString(_(text), ContentTextWidth);
