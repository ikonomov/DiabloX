...
void CheckCursMove()
{
	...
	if (ControlMode != ControlTypes::KeyboardAndMouse && (sgbMouseDown != CLICK_NONE || ControllerActionHeld != GameActionType_NONE) && IsNoneOf(LastMouseButtonAction, MouseActionType::None, MouseActionType::Attack, MouseActionType::Spell)) { // disabled for auto-clicker
		InvalidateTargets();
