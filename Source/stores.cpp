...
void StartBoy()
{
	...
	if (!boyitem.isEmpty()) {
		...
		AddSText(0, 14, fmt::format(fmt::runtime(_("but it will cost {:d} gold")), clamp((((MyPlayer->_pLevel - 1) * 10) / 25 + 1) * 100, 100, 1000)), UiFlags::ColorWhitegold | UiFlags::AlignCenter, false);
...
void BoyEnter()
{
	int entrycost = clamp((((MyPlayer->_pLevel - 1) * 10) / 25 + 1) * 100, 100, 1000);
