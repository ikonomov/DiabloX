...
void DrawItemNameLabels(const Surface &out)
{
	...
	for (const ItemLabel &label : labelQueue) {
		...
		if (pcursitem == label.id && stextflag == TalkID::None)
			FillRect(clippedOut, label.pos.x, label.pos.y, label.width + 2, labelHeight + 3, PAL8_BLUE + 6);
		else
			DrawHalfTransparentRectTo(clippedOut, label.pos.x, label.pos.y, label.width + 2, labelHeight + 3);
		DrawString(clippedOut, label.text, { { label.pos.x + MarginX + 1, label.pos.y + labelMarginTop + 2 }, { label.width, labelHeight } },
		    item.getTextColor());
	}
	labelQueue.clear();
}
